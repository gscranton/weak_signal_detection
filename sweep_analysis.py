...
real_detection_coefficient_flag = bool(int(args.real_detection_coefficient_flag))
...
for i in range(len(amp_factor_num_unique_list)):
    ...
    for k in inds_cur:
        if real_detection_coefficient_flag:
            parameter_list1 = np.append(parameter_list1, np.real(z1_ff_freq_list1[k]) + np.real(z3_ff_freq_list1[k]))
        else:
            parameter_list1 = np.append(parameter_list1, np.abs(z1_ff_freq_list1[k]) - np.abs(z3_ff_freq_list1[k]))
    if real_detection_coefficient_flag:
        temp = np.mean( np.real(z1_temp) + np.real(z3_temp) )
    else:
...
for i in range(len(amp_factor_num_unique_list)):
    ...
    for k in inds_cur:
        if real_detection_coefficient_flag:
            parameter_list0 = np.append(parameter_list0, np.real(z1_ff_freq_list0[k]) + np.real(z3_ff_freq_list0[k]))
        else:
...
fig_run_dir = '/figs_run'+str(run_num)
...
# vs time figure
if True:
    plt2_min = plt2_max - ratio
    plt.figure()
    plt.plot(t0_list[plt2_min:plt2_max],parameter_list0[plt2_min:plt2_max],color=pltcolors[0])
    plt.plot(t0_list[plt2_min:plt2_max],parameter_list1[plt2_min:plt2_max],color=pltcolors[1])
    plt.plot(t0_list[plt2_min:plt2_max],parameter_list0[plt2_min:plt2_max],'x',color=pltcolors[0])
    plt.plot(t0_list[plt2_min:plt2_max],parameter_list1[plt2_min:plt2_max],'x',color=pltcolors[1])
    #plt.xticks(list(np.linspace(0,len(parameter_list0[plt2_min:plt2_max]),(len(amp_factor_unique_list[plt_min:plt_max])+1))))
    plt.legend(["noise","noise+signal"])
